...
export function ExtractedDataReview({ extractedData, sourceSummary, onDataChange, onConfirm }: ExtractedDataReviewProps) {
  return (
    <div className="space-y-6 rounded-3xl border border-slate-200 bg-white p-8 shadow-sm ring-1 ring-slate-200">
      ...
      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
        <p className="text-sm font-medium text-slate-700">Использованные источники</p>
        <p className="mt-2 text-sm text-slate-600">{sourceSummary}</p>
      </div>
      ...
      <p className="text-sm text-slate-500">Если данных недостаточно, AI отметит поля как &quot;Требует проверки&quot; или &quot;Не найдено&quot;.</p>
    </div>
  );
}
